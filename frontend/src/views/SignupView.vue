    ...
    <v-row no-gutters style="height:100vh;overflow:hidden">
              ...
              <v-row style="height:100vh;margin:0;display:flex;flex-direction:column;justify-content:space-between;padding:40px 48px">
                ...
                <div style="flex:1;display:flex;flex-direction:column;justify-content:center">
                  ...
                  <div style="font-family:'Georgia',serif;font-size:48px;font-weight:900;color:white;line-height:1.1;margin-bottom:18px;">
                    Join a World of <span style="color:#BCAAA4;font-style:italic">Creators</span>
                  </div>
                  ...
                  <div style="color:rgba(215,204,200,0.70);font-size:15px;line-height:1.8;max-width:340px;margin-bottom:28px;font-family:sans-serif;">
                    Sell your art to collectors worldwide or discover your next favourite piece.
                  </div>

                  <div style="display:flex;gap:8px;flex-wrap:wrap">
                    <v-chip small outlined color="blue-grey lighten-3">
                      <v-icon x-small left>mdi-upload</v-icon>Upload &amp; Sell
                    </v-chip>
                    <v-chip small outlined color="blue-grey lighten-3">
                      <v-icon x-small left>mdi-shopping</v-icon>Browse &amp; Buy
                    </v-chip>
                    <v-chip small outlined color="blue-grey lighten-3">
                      <v-icon x-small left>mdi-star-outline</v-icon>Free to Join
                    </v-chip>
                  </div>
      ...
      >
        <v-container style="max-width:480px;padding:0 48px;width:100%">
          ...
          <!-- Heading -->
          <div class="font-weight-black mb-1" style="color:#2C1810;font-family:'Georgia',serif;font-size:32px">Create Account</div>
          <div class="body-2 mb-4" style="color:#A1887F;font-size:14px">Join our gallery of artists and collectors</div>
          ...
          <!-- Full Name -->
          <div class="caption font-weight-bold mb-1" style="color:#6D4C41;letter-spacing:.08em;text-transform:uppercase;font-size:11px">Full Name</div>
          <v-text-field
          ...
          <!-- Confirm Password -->
          <div class="caption font-weight-bold mb-1" style="color:#6D4C41;letter-spacing:.08em;text-transform:uppercase;font-size:11px">Confirm Password</div>
          <v-text-field
          ...
          <div class="d-flex align-center mb-4">
            <v-divider />
            <span class="caption px-3" style="color:#BCAAA4;white-space:nowrap;font-size:12px">Already have an account?</span>
            <v-divider />
          </div>
...
export default {
  ...
  methods: {
    submit: function () {
      var self = this
      if (!self.form.fullName || !self.form.email || !self.form.password || !self.form.confirmPassword) {
        self.$store.commit('auth/SET_ERROR', 'Please fill in all fields.')
        return
      }
      if (self.form.password.length < 6) {
        self.$store.commit('auth/SET_ERROR', 'Password must be at least 6 characters.')
        return
      }
      if (self.form.password !== self.form.confirmPassword) {
        self.$store.commit('auth/SET_ERROR', 'Passwords do not match.')
        return
      }
      self.$store.dispatch('auth/register', {
        ...
        .then(function () {
          if (self.role === 'artist') self.$router.push('/artist/dashboard')
